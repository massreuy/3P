...
namespace _3PA.MainFeatures.AutoCompletionFeature {
    ...
    internal static class AutoCompletion {
        ...
        public static void OnShowCompleteSuggestionList() {
            ...
            _shownLine = Npp.Editor.CurrentLine;
            _shownPosition = Npp.Editor.CurrentPosition;
        ...
        public static List<CompletionItem> FindInCompletionData(string keyword, int position, bool dontCheckLine = false) {
            var filteredList = GetSortedFilteredSavedList(Npp.Editor.LineFromPosition(position), dontCheckLine);
            if (filteredList == null || filteredList.Count <= 0) return null;
            ...
            var tableFound = ParserHandler.FindAnyTableOrBufferByName(Npp.Editor.GetFirstWordRightAfterPoint(position));
        ...
        public static void UpdateAutocompletion(bool canChangeListType) {
            ...
            var nppCurrentPosition = Npp.Editor.CurrentPosition;
            var nppCurrentLine = Npp.Editor.CurrentLine;
            ...
            int nbPoints;
            string previousWord = "";
            var strOnLeft = Npp.Editor.GetTextOnLeftOfPos(nppCurrentPosition);
        ...
        private static void ShowSuggestionList(AutoCompletionForm form) {
            ...
            // update position
            var point = Npp.Editor.GetCaretScreenLocation();
            var lineHeight = Npp.Editor.TextHeight(nppCurrentLine);
            point.Y += lineHeight;
        ...
        public static void InsertSuggestion(CompletionItem data, int offset = 0) {
            try {
                ...
                Npp.Editor.ReplaceKeywordWrapped(replacementText, offset);
